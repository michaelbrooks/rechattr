(function() {
    ...
    /* CreateApp.prototype.initializeConstraints = function() {
        var self = this;
        this.$startDate.on('change', function() {
            self.pushStopDateTimeToDuration();
        });
        ...
        this.$stopDatePicker.on('changeDate', function() {
            var stopDate = self.getStopDateTime();
            var startDate = self.getStartDateTime();
            
            self.duration = stopDate.diff(startDate, 'seconds');
        });
        
        this.setDateTimeLimits();
    }
    
    CreateApp.prototype.pushStopDateTimeToDuration = function() {
        
        var startDateTime = this.getStartDateTime();
        var stopDateTime = this.getStopDateTime();
            
        //The later date must not be before the earlier date
        if (startDateTime.isAfter(stopDateTime)) {
        
            startDateTime.add('seconds', this.duration);
            
            this.$stopDatePicker.datepicker('update', startDateTime.format(DATE_FORMAT));
            this.$stopTime.val(startDateTime.format(TIME_FORMAT));
            
            this.setDateTimeLimits();
        }
    }
    
    CreateApp.prototype.setDateTimeLimits = function() {
        var startDateTime = this.getStartDateTime();
        
        this.$stopDatePicker.datepicker('setStartDate', startDateTime.format(DATE_FORMAT));
    } */
